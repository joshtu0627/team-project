...
const FilterButton: React.FC<FilterButtonProps> = ({
  ...
  return (
    <div>
      <button
        className={`border p-2 mx-5 my-2 rounded-3xl text-black font-semibold ${
          filter === title ? "bg-yellow-400" : "hover:bg-yellow-400"
        }`}
...
const EmojiButton: React.FC<EmojiButtonProps> = (props) => {
  ...
  const [emojiAnchorEl, setEmojiAnchorEl] = React.useState<HTMLElement | null>(
    null
  );
  const [numberAnchorEl, setNumberAnchorEl] =
  ...
  const handleNumberPopoverOpen = (event: React.MouseEvent<HTMLElement>) => {
...
const Review: React.FC<ReviewProps> = (props) => {
  ...
  return (
    <div className="p-6 mt-4 border-t border-gray-300">
      <div>
      ...
      <div className="mt-2 mb-2 text-gray-500">
        ...
        <p>
          {height} cm ． {weight} kg ． {style}
        </p>
        <p>
          {color_name} ． {size} ． {size_review}
        </p>
      </div>
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      const { data } = await axios.get(
        `${backendurl}/api/1.0/review/product/${product_id}`
      );
