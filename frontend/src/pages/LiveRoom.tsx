...
function MyVideoConference({
  ...
  const initiatorTrack = tracks.find(
    (track) => track?.participant?.identity === initiatorIdentity
  );
  ...
  const handleCouponClick = async () => {
    ...
    try {
      const response = await fetch(`${backendurl}/api/1.0/user/reward`, {
        method: "POST",
        headers: {
  ...
  return (
    <div style={{ position: "relative", height: "100%" }}>
      <TitleBar>Stylish試衣間 | 限時特惠中</TitleBar>
      ...
      <GridLayout
        tracks={[initiatorTrack]}
        style={{ height: "calc(100vh - var(--lk-control-bar-height))" }}
      >
        {initiatorTrack && (
          <ParticipantTile key={initiatorTrack.sid} track={initiatorTrack} />
        )}
      </GridLayout>
      ...
      {showCoupon && (
        <img
          src={`/assets/images/coupons/15.00.png`}
          alt="優惠卷"
          className="w-60"
          style={{
            position: "absolute",
...
export default function LiveRoom() {
  ...
  const handleSubmitProductID = () => {
    if (user?.id === 10243 && productID && socket) {
      console.log(
        "Emitting broadcastProductId event with productId:",
        productID
      );
